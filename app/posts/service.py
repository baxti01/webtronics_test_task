...
from app.posts.serializer import CreatePost, UpdatePost
...
class PostsService:
    ...
    def get_all(self, user_id: int) -> List[Posts]:
        return self.database.get_all(Posts, user_id=user_id)

    def get(self, user_id: int, post_id: int) -> Posts:
        return self.database.get(Posts, user_id=user_id, id=post_id)
